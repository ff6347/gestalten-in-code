...
class Stroke {
    ...
    addParticle(deltaX, deltaY) {
        ...
        let fx = map(noise(this.dx * noise(cos(400) / 40) * noise(sin(1000) / 40)), 0, 1, 0, WIDTH)
        let fy = map(noise(this.dy), 0, 1, 0, HEIGHT)
        ...
        if (this.particles.length < 2) this.particles.push(new Particle(fx, fy, 5, fx / fy * Math.PI * 2))
        else this.particles.push(new Particle(fx, fy, maxSize(map(dist, 4, 0, 0, 10)), fx / fy * Math.PI * 2))
